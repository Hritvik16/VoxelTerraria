...
namespace VoxelEngine
{
    ...
    public class WorldManager : MonoBehaviour
    {
        ...
        public void GenerateBlueprint()
        {
            ...
            {
                featureBuffer = new ComputeBuffer(mapFeatures.Count, 32);
                featureBuffer.SetData(mapFeatures);
                Shader.SetGlobalBuffer("_FeatureAnchorBuffer", featureBuffer);
                Shader.SetGlobalInt("_FeatureCount", mapFeatures.Count);
            }

            if (cavernNodes.Count > 0)
            {
                cavernBuffer = new ComputeBuffer(cavernNodes.Count, 32);
                cavernBuffer.SetData(cavernNodes);
                Shader.SetGlobalBuffer("_CavernNodeBuffer", cavernBuffer);
                Shader.SetGlobalInt("_CavernCount", cavernNodes.Count);
            }

            if (tunnelSplines.Count > 0)
            {
                tunnelBuffer = new ComputeBuffer(tunnelSplines.Count, 32);
                tunnelBuffer.SetData(tunnelSplines);
