...
int GetMaterial(float3 worldXYZ, float baseHeight, int surfaceBiome) {
    ...
    // We already culled solid hubs to local memory, why call global buffer here? This was call #4.
    // We will pass the pre-calculated biome ID from WorldGen inner loop.
